...
----
{
  "case-data" : {
    "hiringPetition" : {
      "jobTitle": "Business Automation SME",
      "jobDescription": "A nice job with a great company, are you ready for this challenge? This could be your next opportunity",
...
. *Execute* the POST request, and look for the result. The result should include the *Job Id*; (actually is the case instance ID, its prefix configuration is part of the case definition properties.)

FIXME: Current RH-SSO configuration does not allow Swagger execution. It responds as `TypeError: Failed to fetch`
