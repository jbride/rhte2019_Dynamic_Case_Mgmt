...
== Deploy the Fuse Swift Integration
In order to demonstrate integration between RHPAM and external systems, your case management project will consume a REST web service provided by Spring Boot and empowered by Fuse routes to enable communication with Documents stored in the Openstack SWIFT appliance.
...
=== Deployment of the Fuse Swift integration [alternate method]
A Fuse REST service is deployed in the Openshift environment to connect to the Openstack Swift node for document storage and retrieval. This section explains how the Fuse service is deployed.
...
+
----
$ skopeo copy --src-tls-verify=false --dest-tls-verify=false --src-creds=openshift:$(oc whoami -t) --dest-creds=<quay userid>:<quay password> docker://docker-registry-default.apps-a8b2.generic.opentlc.com/swift-integration/document-integration docker://quay.io/nandanj/initialtest
----
...
$ oc get route
$ APP_ROUTE_HOST=<route name of Fuse application>
----
